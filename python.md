...
Python的创始人吉多·范罗苏姆说过这样一句话：”Life is short, you need python”，后来传入国内被翻译成“**人生苦短，我用python**".
...
<center class="half">
    <img src="https://pic4.zhimg.com/v2-8172e0134d8a61872b30354e5b300ffb_b.gif" width=300 height=300/>
    <img src="D:\pythonSharePicture\SVM1.jpg" width=300 height=300/>
</center>
...
Python中单行注释以 **#** 开头，多行注释可以用多个#号，或者''' 和 """：
...
python最具特色的就是使用缩进来表示代码块，一般采用四个空格统一缩进。
...
#示例
str1 = '江苏省苏州市'
str11 = str1[1:4]  #结果为 苏省苏
str12 = str1[-3:-1]  #结果为 苏州
```
...
<img src="D:\pythonSharePicture\list_index.png" alt="list_index" style="zoom:50%;" />

```python
#示例
list = [ 'abcd', 786 , 2.23, '江苏苏州', 70.2 ]
tinylist = [123, '江苏苏州']

print (list)            # 输出完整列表
print (list[0])         # 输出列表第一个元素
print (list[1:3])       # 从第二个开始输出到第三个元素,不包含末尾元素
...
元组创建很简单，只需要在括号中添加元素，并使用逗号隔开即可；创建空元组时，使用括号即可；需要特别注意的是，当元组中只包含一个元素时，需要在元素后面添加逗号，否则括号会被当成运算符使用。
...
tuple3 = (3)
tuple4 = (3,)
print(type(tuple3))  #类型为整型，3后面没加逗号
print(type(tuple4))  #类型为元组
```
...
集合（set）是由一个或数个形态各异的大小整体组成的，构成集合的事物或对象称作元素或是成员。集合是一个无序的不重复元素序列，它具有自动去重功能。

基本功能是进行成员关系测试和删除重复元素。可以使用大括号 **{ }** 或者 **set()** 函数创建集合，注意：创建一个空集合必须用 **set()** 而不是 **{ }**，因为 **{ }** 是用来创建一个空字典。

```python 
#示例
sites = {'Google', 'Taobao', 'huawei', 'Facebook', 'Zhihu', 'Baidu'}
...
向集合中添加元素的方法有两种，add 和 update.

```python
>>> thisset = set(("Google", "huawei", "Taobao"))
>>> thisset.add("Facebook")
>>> print(thisset)
{'Taobao', 'Facebook', 'Google', 'huawei'}

>>> thisset.update({1,3})
>>> print(thisset)
{1, 3, 'Google', 'Taobao', 'huawei'}
>>> thisset.update([1,4],[5,6])  
>>> print(thisset)
{1, 3, 4, 5, 6, 'Google', 'Taobao', 'huawei'}
```

#### 移除元素

移除元组中元素的方法有remove(被移除的元素不存在时，会报错)、discard(元素不存在时不报错)

```python
thisset = set(("Google", "huawei", "Taobao"))
thisset.remove('Google')
thisset.discard('Taobao')
length = len(thisset)  #得到集合中的元素个数
thisset.clear() #清空集合中的元素
```
...
![](D:\pythonSharePicture\dict_pic.png)
...
dict = {} #创建空字典
dict['one'] = "1 - 华为" #向字典中添加新的键值对
dict[2]     = "2 - 谷歌"

tinydict = {'name': '谷歌','code':1, 'site': 'www.google.com'}
...
```python
tinydict = {'Name': 'xianming', 'Age': 7, 'Class': 'First'}
...
![](D:\pythonSharePicture\if_pic.jpg)
...
![](D:\pythonSharePicture\while.png)
...
![](D:\pythonSharePicture\for.jpg)

如果你需要遍历数字序列，可以使用内置range()函数，它会生成数列。

```python
for i in range(5):  #range(5)生成【0，1，2，3，4】
for i in range(0,10,3):  #range生成【0，3，6，9】
```
...
![](D:\pythonSharePicture\hanshu.png)
...
调用函数时可使用的正式参数类型有四种：必需参数、关键字参数、默认参数和不定长参数。
...
#调用printinfo函数
printinfo( age=50, name="xiaoming" )
print ("------------------------")
...
有时我们可能需要一个函数能处理比当初声明时更多的参数。这些参数叫做不定长参数，和上述 2 种参数不同，声明时不会命名。基本语法如下：
...
printinfo(70)  #输出结果为 70
```

还有一种就是参数带两个星号 ***\***，加了两个星号 ***\*** 的参数会以**字典**的形式导入，基本语法如下：
...
# 调用printinfo 函数
printinfo(1, a=2,b=3)  #输出为 1，{'a':2,'b':3}
```
...
class student(people):
    ...
    def __init__(self,n,a,w,g):
        #调用父类的构函
        people.__init__(self,n,a,w)  #也可使用 super.__init__(self,n,a,w)
        self.grade = g
    #覆写父类的方法
    def speak(self):
